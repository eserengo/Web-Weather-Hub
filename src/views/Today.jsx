...
import HourlyTemp from '../components/widgets/HourlyTemp'
import HorlyHum from '../components/widgets/HourlyHum'
      ...
      <section className={`${setClassName()} bg text-offWhite col-12 rounded-4 d-flex flex-column align-items-center align-items-sm-start justify-content-center gap-xs-2 pt-4 p-xs-2`}>
        {weatherData?.current &&
          <>
            ...
            <div className='title d-flex align-items-center gap-2 fw-bold'>
              <span>{weatherData?.current?.temperature_2m} {weatherData?.current_units?.temperature_2m}</span>
              <i className='swap-icon drop d-inline pt-1 pt-sm-2 pt-lg-3' onClick={handleSwapClick}>
              <SwapSvg />
              </i>
            </div>
          <p className='para'><WeatherCode input={weatherData?.current?.weather_code} output={'desc'} isDay={isDay.value} /></p>
          <span className='para'>
            {new Date(weatherData?.current?.time).toLocaleString('en', {
              weekday: 'long',
              day: 'numeric',
              month: 'long',
              timeZone: 'UTC',
      ...
      <section className='widgets col-12'>
        {weatherData?.current &&
          <>
            ...
            <HorlyHum weatherData={weatherData} />
          </>
        }
      </section>
    </>
